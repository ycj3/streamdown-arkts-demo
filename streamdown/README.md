...
- **流式解析**: 字符级增量渲染，支持实时流式内容
- **块级架构**: 通过不可变 diff 实现高效更新
- **富文本支持**:
  - 标题 (`# H1` ~ `###### H6`)
  - 段落（支持行内样式）
  - **粗体** (`**text**`)
...
创建一个新的 `StreamDownController` 实例。

**返回**: `StreamDownController`
...
| 属性         | 类型                   | 说明       |
| ------------ | ---------------------- | ---------- |
| `controller` | `StreamDownController` | 控制器实例 |
...
数据输入      解析器          差异更新       UI 组件
   ...
   ▼            ▼               ▼            ▼
┌────────┐  ┌──────────┐    ┌────────┐   ┌──────────┐
│ 字符流  │─▶│BlockReducer│─▶│BlockDiff│──▶│StreamDown│
